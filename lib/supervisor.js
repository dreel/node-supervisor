...
function run (args) {
  var arg, next, watch, program, extensions, executor, watchDelay;
...
function help () {
  print
    ...
    ("")
    ("  -d|--delay <watchDelay>")
    ("    How long in ms to delay the reload after watch detects a change.")
    ("    Used when several files will change at once to prevent")
    ("    triggering the crash loop detector.")
    ("    Default is 0")
    ("")
    ("  -h|--help|-?")
...
var delayTimer = null, delayComplete = false;
function crash (oldStat, newStat) {
  ...
  if (delayTimer) {
    return;
  } else if (watchDelay > 0) {
    delayTimer = setTimeout(endDelay, watchDelay);
  }
